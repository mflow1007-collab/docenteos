  ...
  return (
    <div className="diagnostico-page">
      ...
      {paso === 1 && <section className="diag-panel">
        ...
        <div className={`diag-curriculo-status ${referentes.oficial ? "oficial" : "revision"}`}><div><strong>{cargandoReferentes ? "Consultando currículo…" : referentes.indicadores.length ? `${referentes.indicadores.length} indicadores disponibles` : "Sin indicadores vinculados"}</strong><p>{referentes.anterior ? `Saberes de entrada tomados de ${referentes.anterior.etiqueta}. ` : ""}{referentes.fuente}</p></div>{!referentes.oficial && referentes.indicadores.length > 0 && <span>Revisar antes de aplicar</span>}</div>
