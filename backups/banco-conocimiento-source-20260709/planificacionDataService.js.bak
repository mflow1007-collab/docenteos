 ...
 * construida desde el Diseño Curricular MINERD importado (diseñoCurricular),
...
/**
 * Busca el documento de diseñoCurricular que corresponde al plan.
 * Prueba área directa y el mapeo asignatura → área curricular.
 * Devuelve null si no hay currículo importado (la capa se genera igual,
 * con IDs locales estables, y se puede re-vincular después).
 */
export const consultarCurriculoParaPlan = async (planificacion) => {
  ...
  for (const area of candidatas) {
    const docCurriculo = await consultarCurriculo(nivel, grado, area);
    if (docCurriculo) return docCurriculo;
  }
  return null;
};
